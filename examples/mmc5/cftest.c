...
extern const char cave_item_fanfare[];
extern const char credits[];
extern const char crystal_fanfare[];
extern const char final_boss[];
extern const char gp[];
extern const char house[];
extern const char overworld[];
extern const char palace[];
extern const char town_item_fanfare[];
extern const char town[];
extern const char triforce[];
extern const char zelda[];
...
void play_song(uint8_t player_pad) {
    ...
    if (player_pad & 0x30) {
        if (player_pad & 0x10) {
            which -= 1;
            if (which < 0) which = ARRAY_SIZE(song) - 1;
        } else {
            which += 1;
            if (which >= ARRAY_SIZE(song)) which = 0;
        }
        cfplayer_reset_song();
        cfplayer_now_playing = song[which];
...
void main(void)
{
    writereg8(0x5102, 2);
    writereg8(0x5103, 1);
    writereg8(0x5113, 0x80);
    cfplayer_init();
    //cfplayer_now_playing = overworld_textpb;
    //cfplayer_now_playing = gp_textpb;
    ...
    vram_puts(0, 3, "CFplayer MMC5 test ROM.");
    vram_puts(0, 4, "Frame ");
